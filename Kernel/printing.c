...
void setCursor(const struct point_t* cursor)
{
	...
	screenStates[screenIndex].cursor.row = (cursor->row < screenStates[screenIndex].topLeft.row)? screenStates[screenIndex].topLeft.row : cursor->row;
	screenStates[screenIndex].cursor.column = (cursor->column < screenStates[screenIndex].topLeft.column)? screenStates[screenIndex].topLeft.column : cursor->column;
}
